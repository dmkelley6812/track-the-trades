...
export default function MessageBubble({ message }) {
    ...
    return (
        <div className={cn("flex gap-3", isUser ? "justify-end" : "justify-start")}>
            {!isUser && (
                <div className="h-8 w-8 rounded-lg bg-emerald-500/20 flex items-center justify-center mt-0.5 border border-emerald-500/30">
            ...
            <div className={cn("max-w-[85%]", isUser && "flex flex-col items-end")}>
                {message.content && (
                    <div className={cn(
                        ...
                        isUser ? "bg-emerald-600 text-white" : "bg-slate-800/50 border border-slate-700 text-slate-100"
                    )}>
                        {isUser ? (
                            ...
                            <ReactMarkdown 
                                ...
                                components={{
                                        ...
                                        return !inline && match ? (
                                            <div className="relative group/code">
                                                ...
                                                <Button
                                                    size="icon"
                                                    variant="ghost"
                                                    className="absolute top-2 right-2 h-6 w-6 opacity-0 group-hover/code:opacity-100 bg-slate-900 hover:bg-slate-800"
                                                    onClick={() => {
                                            ...
                                            <code className="px-1 py-0.5 rounded bg-slate-900 text-emerald-300 text-xs">
                                                {children}
                                            </code>
                                        );
                                    },
                                    a: ({ children, ...props }) => (
                                        <a {...props} target="_blank" rel="noopener noreferrer" className="text-emerald-400 hover:text-emerald-300">{children}</a>
                                    ),
                                    ...
                                    blockquote: ({ children }) => (
                                        <blockquote className="border-l-2 border-emerald-500 pl-3 my-2 text-slate-300">
                                            {children}
                                        </blockquote>
                                    ),
                                }}
